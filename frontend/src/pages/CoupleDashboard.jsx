...
function DashboardUI() {
  ...
  const handlePasswordChange = async (e) => {
    e.preventDefault();
    setPassMessage(''); setPassError(false); setIsUpdating(true);
    try {
      const token = localStorage.getItem('accessToken');
      ...
      setPassError(true); setPassMessage(err.message);
  ...
  const fetchProfile = async () => {
    try {
      ...
      if(data.success && data.profile) {
        setProfileData({ partnerName: data.profile.partnerName||'', weddingDate: data.profile.weddingDate||'', guestCount: data.profile.guestCount||'', style: data.profile.style||'', budget: data.profile.budget||'', priority: data.profile.priority||'' });
      }
    } catch(e){}
  };
  ...
  const handleSendMsg = async () => {
    ...
    try {
      ...
      setNewMsgContent(''); fetchChat(activeChatId);
    } catch(e){}
  };
  ...
  useEffect(() => { if(activeChatId) fetchChat(activeChatId); }, [activeChatId]);
    ...
    <section className="py-20 px-6 bg-wedding-blush/10 min-h-[80vh]">
      <div className="max-w-6xl mx-auto">
        <header className="mb-10">
          ...
          <p className="text-gray-500 uppercase tracking-widest text-sm">Manage your wedding journey</p>
        </header>
        ...
        <div className="grid md:grid-cols-[1fr_2fr] gap-8">
          <div className="space-y-6">
            ...
            <div className="bg-white border border-gray-100 shadow-sm p-6 overflow-hidden rounded-md">
              ...
              <ul className="text-sm space-y-3 text-gray-600">
                <li className="flex items-start gap-3"><div className="mt-0.5"><input type="checkbox" className="accent-wedding-gold w-4 h-4" defaultChecked readOnly /></div><span className="text-gray-400 line-through">Create Planora Account</span></li>
                <li className="flex items-start gap-3"><div className="mt-0.5"><input type="checkbox" className="accent-wedding-gold w-4 h-4" defaultChecked readOnly /></div><span className="text-gray-400 line-through">Set initial budget criteria</span></li>
                <li className="flex items-start gap-3"><div className="mt-0.5"><input type="checkbox" className="accent-wedding-gold w-4 h-4" readOnly /></div><span className="text-gray-700">Message top 3 venue choices</span></li>
                <li className="flex items-start gap-3"><div className="mt-0.5"><input type="checkbox" className="accent-wedding-gold w-4 h-4" readOnly /></div><span className="text-gray-700">Confirm primary photographer</span></li>
              </ul>
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-100 shadow-sm flex flex-col overflow-hidden rounded-md h-[500px]">
            <div className="p-6 border-b border-gray-100 flex gap-6 text-sm font-medium text-gray-500 bg-gray-50">
              {[['bookings',`Active Bookings (${activeBookings.length})`],['requests',`Pending (${pendingBookings.length})`],['messages','Messages'],['profile','Profile Settings']].map(([id, label]) => (
                <button key={id} onClick={() => setActiveTab(id)} className={activeTab===id ? 'text-wedding-gold border-b-2 border-wedding-gold pb-1' : 'hover:text-gray-800 transition pb-1'}>{label}</button>
              ))}
            </div>
              ...
              {activeTab === 'messages' && (
                <div className="animate-fade-in h-full flex overflow-hidden">
                  ...
                  <div className="flex-1 flex flex-col h-full pl-2">
                          ...
                          {messages.map(m => {
                            ...
                            return (
                              <div key={m._id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                <div className={`px-4 py-2 text-sm max-w-[85%] ${isMe ? 'bg-wedding-cream text-gray-900 border border-wedding-gold/30 rounded-tl-xl rounded-b-xl rounded-tr-sm' : 'bg-gray-100 text-gray-800 border border-gray-200 rounded-tr-xl rounded-b-xl rounded-tl-sm'}`}>{m.content}</div>
                              </div>
                            );
                          })}
                        </div>
                    ...
                    ) : <div className="flex-1 flex items-center justify-center text-gray-400 text-sm italic">Select a conversation</div>}
                  </div>
                </div>
              )}
                ...
                <div className="animate-fade-in flex flex-col h-full overflow-y-auto">
                  ...
                  <form onSubmit={saveProfile}>
                    <div className="grid grid-cols-2 gap-4 mb-4">
                      ...
                      <div><label className="text-xs text-gray-500 uppercase tracking-widest font-semibold block mb-1">Partner Name</label><input type="text" value={profileData.partnerName} onChange={e=>setProfileData({...profileData,partnerName:e.target.value})} className="w-full border border-gray-200 p-2 text-sm focus:outline-none focus:border-wedding-gold transition" /></div>
                      <div><label className="text-xs text-gray-500 uppercase tracking-widest font-semibold block mb-1">Wedding Date / Season</label><input type="text" value={profileData.weddingDate} onChange={e=>setProfileData({...profileData,weddingDate:e.target.value})} className="w-full border border-gray-200 p-2 text-sm focus:outline-none focus:border-wedding-gold transition" /></div>
                      <div><label className="text-xs text-gray-500 uppercase tracking-widest font-semibold block mb-1">Budget (INR)</label><input type="number" value={profileData.budget} onChange={e=>setProfileData({...profileData,budget:e.target.value})} className="w-full border border-gray-200 p-2 text-sm focus:outline-none focus:border-wedding-gold transition" /></div>
                      <div><label className="text-xs text-gray-500 uppercase tracking-widest font-semibold block mb-1">Guest Count</label>
                      ...
                      <div><label className="text-xs text-gray-500 uppercase tracking-widest font-semibold block mb-1">Style Preference</label>
                        <select value={profileData.style} onChange={e=>setProfileData({...profileData,style:e.target.value})} className="w-full border border-gray-200 p-2 text-sm focus:outline-none focus:border-wedding-gold transition bg-white">
                          <option value="">Select</option>
                          <option>Palatial Heritage</option>
                          <option>Modern Minimalist</option>
                          <option>Bohemian Beach</option>
                          <option>Classic Glamour</option>
                        </select>
                      </div>
                      <div><label className="text-xs text-gray-500 uppercase tracking-widest font-semibold block mb-1">Top Priority</label>
                        <select value={profileData.priority} onChange={e=>setProfileData({...profileData,priority:e.target.value})} className="w-full border border-gray-200 p-2 text-sm focus:outline-none focus:border-wedding-gold transition bg-white">
                          <option value="">Select</option>
                          <option>The Venue</option>
                          <option>Design &amp; Decor</option>
                  ...
                  <form onSubmit={handlePasswordChange} className="space-y-4">
                    {passMessage && <div className={`p-3 text-sm rounded-sm border ${passError ? 'bg-red-50 text-red-600 border-red-200' : 'bg-green-50 text-green-700 border-green-200'}`}>{passMessage}</div>}
                    <div><label className="text-xs text-gray-500 uppercase tracking-widest font-semibold block mb-1">Current Password</label><input type="password" required value={oldPassword} onChange={e=>setOldPassword(e.target.value)} className="w-full border border-gray-200 p-2 text-sm focus:outline-none focus:border-wedding-gold transition" /></div>
                    <div><label className="text-xs text-gray-500 uppercase tracking-widest font-semibold block mb-1">New Password</label><input type="password" required value={newPassword} onChange={e=>setNewPassword(e.target.value)} className="w-full border border-gray-200 p-2 text-sm focus:outline-none focus:border-wedding-gold transition" /></div>
                    <button type="submit" disabled={isUpdating} className="bg-wedding-gold text-white px-6 py-2 mt-4 rounded-sm text-sm font-medium hover:bg-yellow-600 transition tracking-wide uppercase">{isUpdating ? 'Updating...' : 'Change Password'}</button>
                  </form>
                </div>
              )}
            </div>
